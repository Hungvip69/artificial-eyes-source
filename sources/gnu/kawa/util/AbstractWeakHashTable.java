...
public abstract class AbstractWeakHashTable<K, V> extends AbstractHashTable<WEntry<K, V>, K, V> {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    @Override // gnu.kawa.util.AbstractHashTable
    public WEntry<K, V> makeEntry(K key, int hash, V value) {
    ...
    public V put(K key, V value) {
        ...
        WEntry<K, V> node2 = makeEntry((Object) null, hash, (Object) value);
        node2.next = first;
    ...
    static <Entry extends Map.Entry<K, V>, K, V> void cleanup(AbstractHashTable<Entry, ?, ?> abstractHashTable, ReferenceQueue<?> referenceQueue) {
        while (true) {
            ...
            if (entry != null) {
                int iHashToIndex = abstractHashTable.hashToIndex(abstractHashTable.getEntryHashCode(entry));
                Map.Entry entry2 = null;
                Map.Entry entry3 = abstractHashTable.table[iHashToIndex];
                while (true) {
                    if (entry3 != null) {
                        Map.Entry entryNext = abstractHashTable.getEntryNext(entry3);
                        if (entry3 == entry) {
                            if (entry2 == null) {
                                ((Entry[]) abstractHashTable.table)[iHashToIndex] = entryNext;
                            } else {
    ...
    public static class WEntry<K, V> extends WeakReference<V> implements Map.Entry<K, V> {
        ...
        public K getKey() {
            ...
            return this.htable.getKeyFromValue((V) obj);
        }
