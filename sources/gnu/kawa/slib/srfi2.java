...
/* JADX INFO: compiled from: srfi2.scm */
...
public class srfi2 extends ModuleBody {
    ...
    static Object lambda1(Object form) {
        ...
        if (Lit1.match(form, objArrAllocVars, 0)) {
            return Lit2.execute(objArrAllocVars, TemplateScope.make());
        }
        if (Lit3.match(form, objArrAllocVars, 0)) {
            return Lit4.execute(objArrAllocVars, TemplateScope.make());
        }
        if (Lit5.match(form, objArrAllocVars, 0)) {
            return Lit6.execute(objArrAllocVars, TemplateScope.make());
        }
        if (Lit7.match(form, objArrAllocVars, 0)) {
            if (std_syntax.isIdentifier(Lit8.execute(objArrAllocVars, TemplateScope.make()))) {
                return Lit9.execute(objArrAllocVars, TemplateScope.make());
            }
            return prim_syntax.syntaxError(Lit10.execute(objArrAllocVars, TemplateScope.make()), "expected a variable name" instanceof Object[] ? (Object[]) "expected a variable name" : new Object[]{"expected a variable name"});
        }
        if (Lit11.match(form, objArrAllocVars, 0)) {
            return Lit12.execute(objArrAllocVars, TemplateScope.make());
        }
        if (Lit13.match(form, objArrAllocVars, 0)) {
        ...
        if (std_syntax.isIdentifier(Lit16.execute(objArrAllocVars, TemplateScope.make()))) {
            return Lit17.execute(objArrAllocVars, TemplateScope.make());
        }
        return prim_syntax.syntaxError(Lit18.execute(objArrAllocVars, TemplateScope.make()), "expected a variable name" instanceof Object[] ? (Object[]) "expected a variable name" : new Object[]{"expected a variable name"});
    }
}
