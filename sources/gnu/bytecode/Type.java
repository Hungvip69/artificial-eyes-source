...
public abstract class Type implements java.lang.reflect.Type {
    ...
    public static synchronized Type make(Class cls) {
        Type typeMake;
        Type type;
        if (mapClassToType != null && (type = mapClassToType.get(cls)) != null) {
            return type;
        }
        if (cls.isArray()) {
            typeMake = ArrayType.make(make(cls.getComponentType()));
        } else {
            if (cls.isPrimitive()) {
            ...
            HashMap<String, Type> map = mapNameToType;
            synchronized (map) {
                Type type2 = map.get(name);
                if (type2 == null || (type2.reflectClass != cls && type2.reflectClass != null)) {
        ...
        registerTypeForClass(cls, typeMake);
        return typeMake;
    }
