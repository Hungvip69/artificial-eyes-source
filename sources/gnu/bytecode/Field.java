        ...
        switch (getType().getSignature().charAt(0)) {
            case 'C':
                if (obj instanceof Character) {
                    cpoolEntryAddInt = constantPool.addInt(((Character) obj).charValue());
                    break;
                }
            case 'B':
            case 'I':
            case 'S':
                cpoolEntryAddInt = constantPool.addInt(((Number) obj).intValue());
                break;
            case 'D':
                cpoolEntryAddInt = constantPool.addDouble(((Number) obj).doubleValue());
                break;
            case 'F':
                cpoolEntryAddInt = constantPool.addFloat(((Number) obj).floatValue());
                break;
            case 'J':
                cpoolEntryAddInt = constantPool.addLong(((Number) obj).longValue());
                break;
            case 'Z':
                cpoolEntryAddInt = constantPool.addInt(PrimType.booleanValue(obj) ? 1 : 0);
                break;
            default:
                cpoolEntryAddInt = constantPool.addString(obj.toString());
                break;
        }
        new ConstantValueAttr(cpoolEntryAddInt.getIndex()).addToFrontOf(this);
    }
