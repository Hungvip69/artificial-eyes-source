...
public class PrettyWriter extends Writer {
    ...
    public void endLogicalBlock() {
        ...
        String suffix = this.queueStrings[suffixLength2 + 6];
        if (suffix != null) {
            write(suffix);
        }
    ...
    void expandTabs(int through) {
        ...
        if (numInsertions > 0) {
            ...
            while (true) {
                ...
                int srcpos = prettyWriter.blocks[blocksUsed + (i * 2)];
                int column2 = column;
                int amount = prettyWriter.blocks[blocksUsed + (i * 2) + 1];
                int dstpos = srcpos + additional;
                int sectionStart2 = sectionStart;
                int sectionStart3 = end - srcpos;
                System.arraycopy(buffer, srcpos, newBuffer, dstpos, sectionStart3);
