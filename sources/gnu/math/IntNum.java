...
public class IntNum extends RatNum implements Externalizable {
    ...
    public void realloc(int nwords) {
        ...
        if (this.words == null || this.words.length < nwords || this.words.length > nwords + 2) {
    ...
    public IntNum canonicalize() {
        if (this.words != null) {
            int iWordsNeeded = wordsNeeded(this.words, this.ival);
            this.ival = iWordsNeeded;
            if (iWordsNeeded <= 1) {
                if (this.ival == 1) {
                    this.ival = this.words[0];
                }
        ...
        if (rlen > 1 || ywords[0] != 0) {
            ...
            switch (rounding_mode2) {
                ...
                case 4:
                    ...
                    add_one2 = cmp == 1 || (cmp == 0 && (xwords[0] & 1) != 0);
                    break;
                default:
                    add_one = false;
                    add_one2 = add_one;
    ...
    public double roundToDouble(int exp, boolean neg, boolean remainder) {
        ...
        int exp3 = exp2 + 1023;
        long bits_exp = exp3 > 0 ? ((long) exp3) << 52 : 0L;
