...
public class DQuantity extends Quantity implements Externalizable {
    ...
    public Numeric add(Object y, int k) {
        ...
        if (dimensions() == Dimensions.Empty && (y instanceof RealNum)) {
            ...
            double d2 = k;
            double dDoubleValue = ((RealNum) y).doubleValue();
            Double.isNaN(d2);
            return new DQuantity(d + (d2 * dDoubleValue), unit());
        }
    ...
    public Numeric addReversed(Numeric x, int k) {
        if (dimensions() == Dimensions.Empty && (x instanceof RealNum)) {
            double dDoubleValue = ((RealNum) x).doubleValue();
            double d = k;
            double d2 = this.factor;
            Double.isNaN(d);
            return new DFloNum(dDoubleValue + (d * d2));
        }
        throw new IllegalArgumentException();
    }
