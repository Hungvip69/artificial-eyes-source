...
public abstract class Quantity extends Numeric {
    ...
    public static Quantity add(Quantity x, Quantity y, int k) {
        ...
        double dReValue = x.reValue();
        double d = k;
        double dReValue2 = y.reValue();
        Double.isNaN(d);
        double re = (dReValue + (d * dReValue2)) / x_factor;
        double dImValue = x.imValue();
        double d2 = k;
        double dImValue2 = y.imValue();
        Double.isNaN(d2);
        double im = (dImValue + (d2 * dImValue2)) / x_factor;
