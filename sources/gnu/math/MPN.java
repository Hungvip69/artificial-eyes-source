...
class MPN {
        ...
        if (D < 0) {
            ...
            if (a1 < b12 || (a1 >> 1) < b12) {
                ...
                if ((D & 1) == 0) {
                    ...
                    r2 = r4 - q;
                    b1 = q;
                } else if (q - r4 <= (((long) D) & 4294967295L)) {
                    r2 = (r4 - q) + ((long) D);
                    b1 = q - 1;
                } else {
                    r2 = (r4 - q) + ((long) D) + ((long) D);
                    b1 = q - 2;
                }
    ...
    public static int divmod_1(int[] quotient, int[] dividend, int len, int divisor) {
        ...
        while (i >= 0) {
            int n0 = dividend[i];
            r = udiv_qrnnd(((-4294967296L) & r) | (((long) n0) & 4294967295L), divisor);
            quotient[i] = (int) r;
            i--;
        }
        return (int) (r >> 32);
    ...
    public static void divide(int[] zds, int nx, int[] y, int ny) {
        ...
        do {
            if (zds[j] == y[ny - 1]) {
                ...
                int qhat2 = zds[j];
                long w = (((long) qhat2) << 32) + (((long) zds[j - 1]) & 4294967295L);
    ...
    public static int set_str(int[] iArr, byte[] bArr, int str_len, int base) {
        int iAdd_1;
        int size = 0;
        if (((base - 1) & base) == 0) {
            int next_bitpos = 0;
        ...
        while (res_digit2 < str_len) {
            ...
            if (size == 0) {
                ...
                int cy_limb = mul_1(iArr, iArr, size, big_base);
                iAdd_1 = cy_limb + add_1(iArr, iArr, size, i4);
            }
            if (iAdd_1 != 0) {
                iArr[size] = iAdd_1;
                size++;
