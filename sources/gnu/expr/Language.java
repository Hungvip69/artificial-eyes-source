...
public abstract class Language {
    ...
    public abstract boolean parse(Compilation compilation, int i) throws SyntaxException, IOException;
    ...
    public static Language getInstance(String langName, Class langClass) {
        ...
        try {
            ...
            try {
                ...
                method = langClass.getDeclaredMethod(methodName, clsArr);
            } catch (Exception e) {
                method = langClass.getDeclaredMethod("getInstance", clsArr);
            }
    ...
    public final Environment getNewEnvironment() {
        StringBuilder sbAppend = new StringBuilder().append("environment-");
        int i = envCounter + 1;
        envCounter = i;
        return Environment.make(sbAppend.append(i).toString(), this.environ);
    }
    ...
    public final Compilation parse(InPort port, SourceMessages messages, int options) throws SyntaxException, IOException {
        return parse(getLexer(port, messages), options, (ModuleInfo) null);
    }

    public final Compilation parse(InPort port, SourceMessages messages, ModuleInfo info) throws SyntaxException, IOException {
        return parse(getLexer(port, messages), 8, info);
    }

    public final Compilation parse(InPort port, SourceMessages messages, int options, ModuleInfo info) throws SyntaxException, IOException {
        return parse(getLexer(port, messages), options, info);
    }

    public final Compilation parse(Lexer lexer, int options, ModuleInfo info) throws SyntaxException, IOException {
