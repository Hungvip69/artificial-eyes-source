...
public class Lexer extends gnu.text.Lexer {
    ...
    public Double getNumericLiteral(int c) throws IOException {
        ...
        if (digit_seen && this.port.pos < this.port.limit) {
            ...
            if (!Character.isLetterOrDigit(c3) && c3 != '.') {
                double dval = ival >= 0 ? ival : IntNum.valueOf(this.port.buffer, i, this.port.pos - i, radix, false).doubleValue();
    ...
    public String getStringLiteral(char quote) throws SyntaxException, IOException {
        int i = this.port.pos;
    ...
    public Object maybeAssignment(Object token) throws SyntaxException, IOException {
        int ch = read();
        if (ch == 61) {
    ...
    public static Object getToken(InPort inp) throws SyntaxException, IOException {
