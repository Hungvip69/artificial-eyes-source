...
public final class StatsAccumulator {
    ...
    public void addAll(Stats values) {
        ...
        if (Doubles.isFinite(this.mean) && Doubles.isFinite(values.mean())) {
            ...
            double d2 = this.count;
            Double.isNaN(d2);
            this.mean = d + ((dCount * delta) / d2);
            double d3 = this.sumOfSquaresOfDeltas;
            double dSumOfSquaresOfDeltas = values.sumOfSquaresOfDeltas();
            double dMean = (values.mean() - this.mean) * delta;
            double dCount2 = values.count();
            Double.isNaN(dCount2);
            this.sumOfSquaresOfDeltas = d3 + dSumOfSquaresOfDeltas + (dMean * dCount2);
        } else {
