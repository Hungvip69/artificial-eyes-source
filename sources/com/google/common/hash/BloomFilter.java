...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
    ...
    public long approximateElementCount() {
        ...
        Double.isNaN(d);
        Double.isNaN(d2);
        double fractionOfBitsSet = d / d2;
        double d3 = -Math.log1p(-fractionOfBitsSet);
        double d4 = bitSize;
        Double.isNaN(d4);
        double d5 = d3 * d4;
        double d6 = this.numHashFunctions;
        Double.isNaN(d6);
        return DoubleMath.roundToLong(d5 / d6, RoundingMode.HALF_UP);
    }
    ...
    static long optimalNumOfBits(long n, double p) {
        ...
        double d = -n;
        double dLog = Math.log(p);
        Double.isNaN(d);
        return (long) ((d * dLog) / (Math.log(2.0d) * Math.log(2.0d)));
    }
