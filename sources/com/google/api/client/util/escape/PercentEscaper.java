...
public class PercentEscaper extends UnicodeEscaper {
    ...
    protected char[] escape(int cp) {
        ...
        if (cp <= 1114111) {
            char[] dest4 = {'%', Access.FIELD_CONTEXT, UPPER_HEX_DIGITS[(cp >>> 2) & 7], '%', UPPER_HEX_DIGITS[(cp & 3) | 8], UPPER_HEX_DIGITS[cp & 15], '%', UPPER_HEX_DIGITS[(cp & 3) | 8], UPPER_HEX_DIGITS[cp & 15], '%', UPPER_HEX_DIGITS[(cp & 3) | 8], UPPER_HEX_DIGITS[cp & 15]};
            int cp7 = cp >>> 4;
            int cp8 = cp7 >>> 2;
            int cp9 = cp8 >>> 4;
            int cp10 = cp9 >>> 2;
