...
public final class Ball extends Sprite {
    ...
    protected void onDraw(android.graphics.Canvas canvas) {
        if (this.visible) {
            double d = this.xLeft;
            double dDeviceDensity = this.form.deviceDensity();
            Double.isNaN(dDeviceDensity);
            float correctedXLeft = (float) (d * dDeviceDensity);
            double d2 = this.yTop;
            double dDeviceDensity2 = this.form.deviceDensity();
            Double.isNaN(dDeviceDensity2);
            float correctedYTop = (float) (d2 * dDeviceDensity2);
    ...
    Vector2D getCenterVector() {
        ...
        double dWidth = Width();
        Double.isNaN(dWidth);
        double xCenter = d + (dWidth / 2.0d);
        double d2 = this.yTop;
        double dHeight = Height();
        Double.isNaN(dHeight);
        double yCenter = d2 + (dHeight / 2.0d);
        return new Vector2D(xCenter, yCenter);
    }

    double getMinProjection(Vector2D axis) {
        double dDotProduct = Vector2D.dotProduct(getCenterVector(), axis);
        double dRadius = Radius();
        double dMagnitude = axis.magnitude();
        Double.isNaN(dRadius);
        return dDotProduct - (dRadius * dMagnitude);
    }
