...
public class Navigation extends AndroidNonvisibleComponent implements Component {
    ...
    public void RequestDirections() {
        if (this.apiKey.equals("")) {
            this.form.dispatchErrorOccurredEvent(this, HttpHeaders.AUTHORIZATION, ErrorMessages.ERROR_INVALID_API_KEY, new Object[0]);
            return;
        }
    ...
    public void performRequest(GeoPoint start, GeoPoint end, TransportMethod method) throws Throwable {
        String finalUrl = this.serviceUrl + method.toUnderlyingValue() + "/geojson";
        ?? url = new URL(finalUrl);
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
        connection.setDoInput(true);
        connection.setDoOutput(true);
        connection.setRequestProperty(HttpHeaders.CONTENT_TYPE, Json.MEDIA_TYPE);
        connection.setRequestMethod(HttpMethods.POST);
        connection.setRequestProperty(HttpHeaders.AUTHORIZATION, this.apiKey);
        try {
            try {
                ...
                try {
                    try {
                        ...
                        YailList features = (YailList) response.get(ComponentDescriptorConstants.FEATURES_TARGET);
                        if (features.size() > 0) {
    ...
    private List<?> getDirections(YailDictionary feature) {
        ...
        return YailDictionary.walkKeyPath(feature, Arrays.asList("properties", "segments", obj, "steps", obj, "instruction"));
    }
}
