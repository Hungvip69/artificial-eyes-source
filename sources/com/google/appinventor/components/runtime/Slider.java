...
public class Slider extends AndroidViewComponent implements SeekBar.OnSeekBarChangeListener {
    ...
    private void setSeekbarPosition() {
        float seekbarPosition = ((this.thumbPosition - this.minValue) / (this.maxValue - this.minValue)) * this.numberOfSteps;
    ...
    @SimpleProperty(description = "Sets the minimum value of slider. If the new minimum is greater than the current maximum, then minimum and maximum will both be set to this value.", userVisible = SyntaxForms.DEBUGGING)
    ...
    public void MinValue(float value) {
        ...
        this.thumbPosition = (((this.maxValue - this.minValue) * this.seekbar.getProgress()) / this.numberOfSteps) + this.minValue;
    }
    ...
    @SimpleProperty(description = "Sets the maximum value of slider. If the new maximum is less than the current minimum, then minimum and maximum will both be set to this value. ", userVisible = SyntaxForms.DEBUGGING)
    ...
    @SimpleProperty(category = PropertyCategory.APPEARANCE, description = "Number of steps on the slider scale. Combined withMinValue and MaxValue, it allows you to get the slider precision that you want, e.g. MinValue = 0,MaxValue = 150, NumberOfSteps = 1000, the slider will change position every 0.15.", userVisible = SyntaxForms.DEBUGGING)
    ...
    @SimpleProperty
    @DesignerProperty(defaultValue = "&HFF444444", editorType = PropertyTypeConstants.PROPERTY_TYPE_COLOR)
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        if (this.notice) {
            this.thumbPosition = (((this.maxValue - this.minValue) * progress) / this.numberOfSteps) + this.minValue;
