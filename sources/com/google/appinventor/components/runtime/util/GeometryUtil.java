...
public final class GeometryUtil {
    ...
    public static Coordinate[] pointsToCoordinates(List<GeoPoint> list) {
        boolean zEquals = list.get(0).equals(list.get(list.size() - 1));
        Coordinate[] coordinateArr = new Coordinate[list.size() + (!zEquals ? 1 : 0)];
        int i = 0;
    ...
    public static double distanceBetween(IGeoPoint a, IGeoPoint b) {
        ...
        double dSqrt = Math.sqrt(cordlen);
        double lng12 = Math.sqrt(1.0d - cordlen);
        double angle = Math.atan2(dSqrt, lng12) * 2.0d;
    ...
    public static double distanceBetweenEdges(MapFactory.MapMarker marker, MapFactory.MapCircle circle) {
        double dDistanceTo = marker.getCentroid().distanceTo(circle.getCentroid());
        double dRadius = circle.Radius();
        Double.isNaN(dDistanceTo);
        double d = dDistanceTo - dRadius;
        if (d < 0.0d) {
            return 0.0d;
        }
        return d;
