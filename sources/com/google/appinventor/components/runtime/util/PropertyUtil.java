        ...
        for (Method componentMethod : componentMethods) {
            if (componentMethod.isAnnotationPresent(SimpleProperty.class) && componentMethod.getParameterTypes().length == 1) {
                try {
                    ...
                    Method propertyCopierMethod = getPropertyCopierMethod("Copy" + propertyName, cls);
                    if (propertyCopierMethod != null) {
                        propertyCopierMethod.invoke(target, source);
                    } else {
                        Method propertyGetterMethod = cls.getMethod(propertyName, new Class[0]);
                        Class<?> cls2 = componentMethod.getParameterTypes()[0];
                        if (propertyGetterMethod.isAnnotationPresent(SimpleProperty.class) && cls2.isAssignableFrom(propertyGetterMethod.getReturnType())) {
