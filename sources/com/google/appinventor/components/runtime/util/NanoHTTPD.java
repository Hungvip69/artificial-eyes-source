...
public class NanoHTTPD {
    ...
    private Thread myThread = new Thread(new Runnable() { // from class: com.google.appinventor.components.runtime.util.NanoHTTPD.1
        ...
        public void run() {
            while (true) {
                try {
                    NanoHTTPD.this.new HTTPSession(NanoHTTPD.this.myServerSocket.accept());
                } catch (IOException e) {
                    return;
                }
            }
    ...
    public Response serve(String uri, String method, Properties header, Properties parms, Properties files, Socket mySocket) {
        ...
        Enumeration<?> enumerationPropertyNames = header.propertyNames();
        while (enumerationPropertyNames.hasMoreElements()) {
            String value = (String) enumerationPropertyNames.nextElement();
            myOut.println("  HDR: '" + value + "' = '" + header.getProperty(value) + "'");
        }
        Enumeration<?> enumerationPropertyNames2 = parms.propertyNames();
        while (enumerationPropertyNames2.hasMoreElements()) {
            String value2 = (String) enumerationPropertyNames2.nextElement();
            myOut.println("  PRM: '" + value2 + "' = '" + parms.getProperty(value2) + "'");
        }
        Enumeration<?> enumerationPropertyNames3 = files.propertyNames();
        while (enumerationPropertyNames3.hasMoreElements()) {
            String value3 = (String) enumerationPropertyNames3.nextElement();
    ...
    private class HTTPSession implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if (is != null && (rlen = is.read((buf = new byte[8192]), 0, 8192)) > 0) {
                    ...
                    if (method2.equalsIgnoreCase(HttpMethods.PUT)) {
                    ...
                    if (method2.equalsIgnoreCase(HttpMethods.POST)) {
        ...
        private void sendResponse(String status, String mime, Properties header, InputStream data) {
            try {
                ...
                if (header == null || header.getProperty(HttpHeaders.DATE) == null) {
    ...
    /* JADX WARN: Removed duplicated region for block: B:162:0x05a6 A[Catch: IOException -> 0x05d1, TRY_ENTER, TRY_LEAVE, TryCatch #4 {IOException -> 0x05d1, blocks: (B:156:0x058a, B:162:0x05a6), top: B:190:0x058a }] */
    /* JADX WARN: Removed duplicated region for block: B:210:0x0594 A[EXC_TOP_SPLITTER, SYNTHETIC] */
